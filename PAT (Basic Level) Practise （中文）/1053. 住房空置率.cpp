...
//有毛病的题意啊，是个人都会想应该是在观察期内超过阈值D天啊，为毛你直接拿观察期去比较，害我想了半天哪里错了！！！！！！
#include<iostream>
#include<string>
#include<algorithm>
#include<iomanip>
...
int main()
{
	int n,d,k; 
	int ans[2] = { 0 };
	double e,temp; //住房总套数，低电量阈值，观察期阈值
	cin >> n >> e >> d;
	for (int i = 0; i < n; i++)
	{	
		cin >> k;
	...
	double may  = (double) ans[0] / n * 100;
	double must = (double) ans[1] / n * 100;
	cout << fixed << setprecision(1) << may << "% ";
	cout << fixed << setprecision(1) << must << "%";
	system("pause");
	return 0;
}
